...
class App {
    ...
    async uploadAttachment(input) {
        ...
        try {
            ...
            for (let i = 0; i < files.length; i++) {
                ...
                try {
                    const res = await fetch(`${baseUrl}/attachments/${this.currentReturnId}`, {
                        method: 'POST',
                        body: formData
                    });
                    const result = await res.json();
    ...
    async deleteAttachment(id) {
        ...
        try {
            const res = await fetch(`${baseUrl}/attachment/${id}`, { method: 'DELETE' });
            const result = await res.json();
            if (result.success) {
    ...
    try {
        ...
        const buildConnection = (url) => {
            ...
            this.hubConnection.on("DbChange", (e) => {
                const opMap = { "INSERT": "إدخال", "UPDATE": "تحديث", "DELETE": "حذف" };
                const tableMap = {
                ...
                const op = opMap[e?.operation] || e?.operation || "";
                const table = tableMap[e?.table] || e?.table || "";
                const status = e?.status || "";
                const ts = e?.timestamp || "";
                const msg = `تم ${op} على جدول ${table} عند ${ts} — الحالة: ${status}`;
                if (this.showUnifiedToast) {
                    this.showUnifiedToast(msg, status === 'ناجحة' ? 'success' : 'error', 'تغييرات قاعدة البيانات');
                } else {
                    alert(msg);
                }
            });

            this.hubConnection.on("UpdateData", (source, user) => {
                console.info(`[RealTime] Data update signal received from: ${source} by ${user}`);
                
                if (this.currentUser && user === this.currentUser.fullname) {
                    console.log('[RealTime] Skipping update from self.');
                    return;
                }
                ...
                this.showSignalRNotification(sourceName, user);
            });
 ...
 */
App.prototype.showSignalRNotification = function (sourceName, user) {
    ...
    banner.innerHTML = `
        <div style="display: flex; align-items: center; gap: 15px;">
            <div style="width: 40px; height: 40px; background: rgba(0, 240, 255, 0.1); border-radius: 50%; display: flex; align-items: center; justify-content: center;">
                <i class="fas fa-sync-alt fa-spin" style="color: #00f0ff;"></i>
            </div>
            <div style="font-size: 1.05em;">
                قام المستخدم <strong style="color: #ffc107;">[${user}]</strong> بتحديث بيانات <span style="color: #00f0ff; font-weight: bold;">(${sourceName})</span>. هل تريد مزامنة التغييرات الآن؟
            </div>
        </div>
        <div style="display: flex; gap: 12px;">
            <button id="banner-accept" class="btn" style="background: #00f0ff; color: #0d1623; font-weight: 800; padding: 6px 25px; border-radius: 8px; border: none; cursor: pointer; transition: all 0.2s;">
                <i class="fas fa-check-circle"></i> نعم، مزامنة
            </button>
