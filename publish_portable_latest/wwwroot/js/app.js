...
console.log('App JS Loaded Successfully version 7.7 - Rebuilt Comprehensive Salary Table');
...
renderTable(dataToRender = null, append = false) {
    ...
    async loadFullReturns(page = 1, pageSize = 50, query = '', isSearch = false) {
        ...
        try {
            ...
            const salaryRows = await this.ensureUnifiedSalaryAmounts(response.salaryRecords || []);
            ...
            this.updateUnifiedStats(
                { filteredCount: incentiveRows.length, totalAmount: this.sumUnifiedAmounts(incentiveRows) },
                { totalCount: salaryRows.length, totalAmount: this.sumUnifiedSalaryAmounts(salaryRows) }
            );
    ...
    async ensureUnifiedSalaryAmounts(rows = []) {
        if (!Array.isArray(rows) || rows.length === 0) return rows;

        const hasMissingAmount = rows.some(row => this.parseAmount(this.getUnifiedAmountValue(row)) === 0);
        if (!hasMissingAmount) return rows;
        ...
        return rows.map(row => {
            const currentAmount = this.parseAmount(this.getUnifiedAmountValue(row));
            ...
            const amount = this.getUnifiedAmountValue(source);
            if (amount !== null && amount !== undefined && String(amount).trim() !== '') {
                row['قيمة العملية'] = amount;
                row.Amount = row.Amount ?? amount;
                row._amount = this.parseAmount(amount);
            }
            return row;
        });
    }
    ...
    renderUnifiedSalaryTable(data) {
        ...
        tableHead.innerHTML = salaryHeaders.map(h => {
            ...
            const isNameCol = hNorm.includes('الاسم') || hNorm.includes('name') || hNorm.includes('fullname');
            let cls = 'sci-fi-th';
            if (h === '#') cls += ' sticky-seq col-id';
            else if (isNameCol) cls += ' sticky-name col-name';
            else if (h.includes('قيمة العملية') || h.includes('المبلغ')) cls += ' sticky-amount col-amount';
            else if (h === 'الإجراءات') cls += ' col-actions';
        ...
        data.slice(0, 100).forEach((row, rowIndex) => {
            ...
            salaryHeaders.forEach(h => {
                ...
                } else if (h === 'قيمة العملية') {
                    val = this.getUnifiedSalaryAmountValue(row);
                } else if (h === 'حالة التسوية') {
                    const actualVal = String(row[h] || '').trim();
                    if (actualVal === 'تم التسوية' || actualVal === 'تمت التسوية') {
                        val = '<span class="badge-status success">تم التسوية ✅</span>';
                    } else if (actualVal === 'لم يتم التسوية') {
                        val = '<span class="badge-status pending">لم يتم التسوية ⏳</span>';
                    } else {
                        val = this.escapeHtml(actualVal);
                    }
                } else {
                    if ((h.includes('تاريخ') || h.includes('Date')) && val) {
                        val = this.formatDate(val);
                ...
                if (h === 'قيمة العملية' && val !== null && val !== undefined && String(val).trim() !== '') {
                    const num = this.parseAmount(val);
                    val = `<span class="${num >= 0 ? 'amount-positive' : 'amount-negative'}">${num.toLocaleString()}</span>`;
                }

                const rawVal = String(val).replace(/<[^>]*>?/gm, '');
                ...
                const hNorm = h.replace(/ـ/g, '').replace(/[أإآ]/g, 'ا').toLowerCase();
                const isNameCol = hNorm.includes('الاسم') || hNorm.includes('name') || hNorm.includes('fullname');
                if (h === '#') td.classList.add('sci-fi-td', 'sticky-seq', 'col-id');
                else if (isNameCol) {
                    td.classList.add('sci-fi-td', 'sticky-name', 'col-name', 'clickable-name');
                    td.title = 'انقر مرتين للبحث السريع عن هذا الاسم';
                    td.ondblclick = () => this.triggerNameSearch(rawVal.trim());
                } else if (h.includes('قيمة العملية') || h.includes('المبلغ')) {
                    td.classList.add('sci-fi-td', 'sticky-amount', 'col-amount');
                } else {
                    td.classList.add('sci-fi-td');
                }
    ...
    async openUnifiedAccountStatement(personKey = '') {
        ...
        try {
            ...
            if (result.requiresSelection) {
                this.renderUnifiedStatementMatches(result.matches || []);
                return;
            }

            this.currentUnifiedStatement = result;
            this.renderUnifiedAccountStatement(result);
        } catch (error) {
...
App.prototype.loadFromOfflineStorage = async function () {
    this.returnsCache = null;
    this.salaryReturnsCache = null;
    await db.clearLocalCache('returns_data_v2').catch(() => {});
    await db.clearLocalCache('salary_returns_data_v1').catch(() => {});
...
App.prototype.populateReturnsCache = async function () {
    await this.rebuildSearchFilterIndex();
};
...
App.prototype._populateSalaryReturnFilterOptions = function(rows = null) {
    ...
    if (indexFilters) {
        ...
        setOptions('salary-payment-date-filter', (indexFilters.paymentDates || []).map(value => ({ value, label: value })), this.salaryPaymentDateFilterValue);
        return;
    }
