/**
 * نظام التحقق من صحة بيانات المرتدات (Lauf)
...
class ValidationService {
    ...
    reset() {
        ...
        this.recordCounts = new Map(); // لتتبع عدد تكرار كل سجل (الاسم + الحساب)

        // -----------------------------------------------------------
        // قوائم التقارير الخاصة
        // -----------------------------------------------------------
    ...
    getValue(row, keys) {
        for (const key of keys) {
            ...
            if (foundKey && row[foundKey] !== undefined && row[foundKey] !== null) {
                ...
                // إذا كانت القيمة مسافات فقء نعتبرها فارغة
    ...
    validate(data, dbRecords = []) {
        ...
        data.forEach((row, index) => {
            ...
            if (hasRealErrors) {
                ...
                // إذا كان فيه تنبيهات فقء نضيفها للأخطاء لكن السجل يبقى "صحيح"
