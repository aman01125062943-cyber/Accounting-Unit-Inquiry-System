    ...
    public static void MapSalaryReturnsEndpoints(this WebApplication app)
    {
        ...
        app.MapGet("/api/salary-returns/changes", async (DatabaseService db, string? since) => {
            ...
            var systemTotalCount = await conn.ExecuteScalarAsync<int>(
                $@"SELECT COUNT(*) FROM SalaryReturns WHERE IsDeleted = 0 AND COALESCE(IsArchived, 0) = 0 {activeImportSql}",
                parameters);
        ...
        // ==========================================
        app.MapPatch("/salary-returns/{id}", async (int id, HttpContext context, DatabaseService db, IHubContext<NotificationHub> hub) => {
            try {
                using var reader = new StreamReader(context.Request.Body);
                ...
                using var conn = await db.GetOpenConnectionAsync();
                var existingRaw = await conn.QueryFirstOrDefaultAsync<string>("SELECT RawData FROM SalaryReturns WHERE Id = @Id", new { Id = id });
                ...
                await db.AddNotificationEventAsync("SalaryReturns", "\u062a\u0639\u062f\u064a\u0644", id, user, PatchAffectsFilterFields(patch.Keys));

                var updatedRecord = ReadJsonObject(rawBody);
                updatedRecord["id"] = id;
                updatedRecord["UpdatedAt"] = updatedAt;
                updatedRecord["AttachmentCount"] = await conn.ExecuteScalarAsync<int>("SELECT COUNT(DISTINCT Filename) FROM SalaryReturnsImages WHERE ReturnId = @Id", new { Id = id });
                return Results.Ok(new { success = true, record = updatedRecord });
            } catch (Exception ex) {
                return Results.Json(new { success = false, message = ex.Message });
            }
